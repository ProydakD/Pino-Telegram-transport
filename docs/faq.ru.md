...
- Используйте прямое создание транспорта и передавайте поток в `pino`: `const stream = telegramTransport(options); const logger = pino({}, stream);`.
- Считайте `transport.target` режимом только для сериализуемых опций, даже если текущая версия Node.js или Pino допускает дополнительные флаги воркера.
